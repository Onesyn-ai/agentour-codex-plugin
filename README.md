...
2. Enter an Agentour access token (`ak_` for a platform account, legacy `at_`, or `ts_` for a tenant user); the Plugin validates it with `GET /v1/dev/me` and asks again if invalid.
